...
import FooterInfoModal from './footer-info-modal';

type ModalKey = 'about' | 'how-it-works' | 'contact';

type FooterClientProps = {
  about: VercelPage;
  howItWorks: VercelPage;
  contact: VercelPage;
};
...
export default function FooterClient({ about, howItWorks, contact }: FooterClientProps) {
  ...
  const pageForModal = modal === 'about' ? about : modal === 'how-it-works' ? howItWorks : modal === 'contact' ? contact : null;
      ...
      <footer className="border-t border-neutral-200 bg-white text-[13px] text-neutral-600">
        ...
        <div className="border-t border-neutral-200 py-5">
          <div className="mx-auto flex max-w-screen-2xl flex-col items-center gap-2 px-6 text-[12px] text-neutral-500 md:flex-row">
            <p>&copy; {currentYear} A Design Line. All rights reserved.</p>
            <p className="md:ml-auto">Let us treat your brand like our own.</p>
          </div>
        </div>
      </footer>

      <FooterInfoModal open={modal !== null} onClose={close} title={pageForModal?.title ?? ''}>
        {modal === 'contact' && pageForModal ? (
          <>
            <Prose className="max-w-none text-[14px] leading-7 text-neutral-700" html={pageForModal.body} />
            <ContactModalForm />
          </>
        ) : pageForModal ? (
          <Prose className="max-w-none text-[14px] leading-7 text-neutral-700" html={pageForModal.body} />
