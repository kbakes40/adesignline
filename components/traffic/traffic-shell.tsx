...
export function TrafficShell({ data }: Props) {
  const kpis = [
    ...
    data.kpis.viewsPerSession,
    data.kpis.avgSessionDuration,
    data.kpis.percentNewUsers
  ];
        ...
        <div className="grid min-w-0 gap-6 lg:col-span-6">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <ChannelBubbleChart
              ...
              channels={data.channelBySessions}
              mode="sessions"
              dateRangeLabel={data.dateRangeLabel}
            />
            <ChannelBubbleChart
              title="Users by channel"
              channels={data.channelByUsers}
              mode="users"
              dateRangeLabel={data.dateRangeLabel}
            />
          </div>
