import { getTrafficRangeSpec } from './date-ranges';
import {
...
import { getAnalyticsDataClient, getGa4PropertyResourceName, getMissingGa4Env } from './ga4-client';
import type {
  TrafficChannelRow,
...
): Promise<ReturnType<typeof parseAggregateRow>> {
...
async function runChannelBreakdown(
  property: string,
  dateRange: { startDate: string; endDate: string }
): Promise<TrafficChannelRow[]> {
  const client = getAnalyticsDataClient();
  if (!client) throw new Error('GA4 client unavailable');
  const [resp] = await client.runReport({
    property,
...
async function runKeywordTable(
  property: string,
  dateRange: { startDate: string; endDate: string }
...
async function runLandingTable(
  property: string,
  dateRange: { startDate: string; endDate: string }
): Promise<TrafficTableRow[]> {
  const client = getAnalyticsDataClient();
...
function shouldUseLandingFallback(rows: TrafficTableRow[]): boolean {
  ...
  const notSetSessions = rows
    .filter((r) => isNotSetTerm(r.label))
...
function generateSummary(
  ...
  const s = pctChange(prev.sessions, cur.sessions);
  const u = pctChange(prev.totalUsers, cur.totalUsers);
  ...
  const parts: string[] = [];
  parts.push(`In the ${label.toLowerCase()}, the site recorded ${Math.round(cur.sessions).toLocaleString('en-US')} sessions and ${Math.round(cur.totalUsers).toLocaleString('en-US')} users.`);

  if (s != null) {
    if (Math.abs(s) < 0.05) {
      parts.push('Sessions were flat versus the prior window.');
    } else {
      const dir = s < 0 ? 'fell' : 'rose';
      parts.push(`Sessions ${dir} about ${Math.abs(s).toFixed(1)}% versus the prior window.`);
    }
  }
  ...
  if (br != null && br !== 0) {
    engagementBits.push(
      `bounce rate ${br < 0 ? 'decreased' : 'increased'} (visitor ${br < 0 ? 'deeper' : 'shallower'} engagement vs the prior period)`
    );
  }
  if (dur != null && dur !== 0) {
    engagementBits.push(`average session duration ${dur > 0 ? 'increased' : 'decreased'}`);
  }
  if (vps != null && vps !== 0) {
    engagementBits.push(`views per session ${vps > 0 ? 'rose' : 'fell'}`);
  }
  if (pnuDelta != null && Math.abs(pnuDelta) > 1) {
    engagementBits.push(`new-user share moved to about ${pnu.toFixed(1)}%`);
  }

  if (engagementBits.length) {
    parts.push(`Engagement: ${engagementBits.join('; ')}.`);
  }

  parts.push(
    'Figures reflect GA4 property data; use filters in Google Analytics for segment drill-downs.'
  );
...
export async function getTrafficData(range: TrafficDateRange): Promise<TrafficResult> {
  ...
  if (missing.length > 0) {
    return { ok: false, reason: 'setup', missingEnv: missing };
  }

  const property = getGa4PropertyResourceName();
  if (!property) {
    return { ok: false, reason: 'setup', missingEnv: ['GA4_PROPERTY_ID'] };
  }
  ...
  try {
    ...
    const kpis: TrafficDashboardData['kpis'] = {
      bounceRate: buildKpi(
        'Bounce rate',
        formatBounceRateFraction(curAgg.bounceRate),
        curAgg.bounceRate,
        prevAgg.bounceRate,
        false
      ),
      viewsPerSession: buildKpi(
        'Views per session',
        curAgg.screenPageViewsPerSession.toFixed(2),
        curAgg.screenPageViewsPerSession,
        prevAgg.screenPageViewsPerSession,
        true
      ),
      avgSessionDuration: buildKpi(
        'Avg. session duration',
        formatDurationSeconds(curAgg.averageSessionDuration),
        curAgg.averageSessionDuration,
        prevAgg.averageSessionDuration,
        true
      ),
      percentNewUsers: buildKpi(
        '% new users',
        formatPercentRatio(pnu),
        pnu,
        pnuPrev,
        true
      )
    ...
    const usersTrend = trend.map((t) => ({ date: t.date, value: t.users }));
    const sessionsTrend = trend.map((t) => ({ date: t.date, value: t.sessions }));
    ...
    const msg = e instanceof Error ? e.message : 'Google Analytics request failed';
