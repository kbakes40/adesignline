/**
 * Fetches all products from A Design Line Patches catalog
 * https://www.adesignline.com/p/catalog/6efee0e1-a9e7-4c14-bdf7-38e96e9b632b/patches
 * and writes lib/patches-catalog-data.ts
 *
 ...
 *
 * Run: node scripts/import-patches-catalog.mjs
 */
...
const OUT = path.join(__dirname, '../lib/patches-catalog-data.ts');
const CATALOG_URL =
  'https://www.adesignline.com/p/catalog/6efee0e1-a9e7-4c14-bdf7-38e96e9b632b/patches';
...
async function fetchPage(offset) {
  const url = `${CATALOG_URL}?offset=${offset}`;
  const res = await fetch(url, {
    headers: {
      'User-Agent': 'ADL-Clone-Import/1.0 (local dev catalog mirror)'
    }
...
<li>For gradient or photo-real art, steer toward dye sublimated or woven; for 3D brand marks, PVC or embroidered.</li>
</ul>
...
<p>Custom patches are <strong class="text-neutral-900">built to order</strong>. We do not stock finished inventory for arbitrary reorder without a new run.</p>
<p>${prod}</p>
...
function recordToProductBlock(rec) {
  ...
  return `  {
    ...
    tags: ['patches', 'categories', 'promotional-products'],
...
async function main() {
  ...
  for (const r of all) {
    unique.set(String(r.SupplierItemGUID).toLowerCase(), r);
  }
  const records = [...unique.values()];
  console.error(`Unique products: ${records.length} (raw ${all.length})`);
...
/** Patches — mirrored from ${CATALOG_URL} (${records.length} SKUs). Re-run: \`node scripts/import-patches-catalog.mjs\` */
export const patchesCatalogProducts: VercelProduct[] = [
`;
