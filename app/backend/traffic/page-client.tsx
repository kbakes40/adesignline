...
export function TrafficPageClient({ initialResult }: Props) {
  ...
  const handleRangeChange = useCallback(
    (range: TrafficDateRange) => {
      startTransition(() => {
        router.push(`/backend/traffic?range=${range}`);
      });
    },
    [router]
  );
  ...
  return (
    <div className="relative min-h-screen">
      <header className={trafficHeaderBar}>
        <div className="mx-auto flex max-w-[1520px] min-w-0 flex-col gap-5 px-5 py-5 sm:px-8 sm:py-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0 shrink-0 lg:max-w-[42%]">
            <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-orange-200/45">A Design Line</p>
          ...
          <div className="flex min-w-0 flex-wrap items-center gap-3 sm:gap-4">
            <TrafficDateRangeSelector selected={currentRange} onChange={handleRangeChange} />
            <button
              type="button"
      ...
      <div
        className={
          isPending ? 'pointer-events-none opacity-50 transition-opacity duration-300' : 'transition-opacity duration-300'
        }
      >
        {result.ok ? (
          <TrafficShell data={result.data} />
        ) : result.reason === 'setup' ? (
          <SetupState missing={result.missingEnv} />
        ) : (
    ...
    <div className="mx-auto max-w-lg px-5 py-16 sm:px-8">
      <div className="rounded-3xl border border-orange-900/50 bg-[#292524]/60 p-8 shadow-xl backdrop-blur-md">
        ...
        <p className="mt-3 text-sm leading-relaxed text-orange-100/60">
          Set the following environment variables for a service account that has <strong className="text-orange-100/90">Viewer</strong>{' '}
          access to your GA4 property. Traffic data is fetched only on the server.
        </p>
        ...
        <p className="mt-6 text-[12px] text-orange-200/45">
          Use <span className="text-orange-100/70">GA4_PROPERTY_ID</span> (numeric property ID). Store{' '}
          <span className="text-orange-100/70">GOOGLE_PRIVATE_KEY</span> with escaped newlines in production.
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="mx-auto max-w-lg px-5 py-16 sm:px-8">
      <div className="rounded-3xl border border-red-900/40 bg-red-950/20 p-8 backdrop-blur-md" role="alert">
        ...
        <p className="mt-4 text-[12px] text-orange-200/45">
          Confirm the service account is granted access to the GA4 property and the property ID is correct.
        </p>
      </div>
    </div>
  );
